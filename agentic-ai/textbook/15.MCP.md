...
- [3. WHAT - 무엇을 구현하는가?](#3-what---무엇을-구현하는가)
  ...
  - [3.4 변경 알림 구독 실습](#34-변경-알림-구독-실습)
    - [3.4.1 변경 알림 구독](#341-변경-알림-구독)
    - [3.4.2 요청 스코프 알림 (진행률·로그)](#342-요청-스코프-알림-진행률로그)
...
| 2 | [간단한 MCP 구현](#32-간단한-mcp-구현) | [15.mcp/simple](https://github.com/cna-bootcamp/aistudy/tree/main/hands-on/15.mcp/simple) | Tools/Resources/Prompts, 외부 API 연동, Claude Code 연동 |
| 3 | [MRTR 실습](#33-mrtr-실습) | [15.mcp/mrtr](https://github.com/cna-bootcamp/aistudy/tree/main/hands-on/15.mcp/mrtr) | `InputRequiredResult` 3라운드, `Resolve()`, `requestState` 서명·검증 |
| 4 | [변경 알림 구독 실습](#34-변경-알림-구독-실습) | [15.mcp/subscriptions](https://github.com/cna-bootcamp/aistudy/tree/main/hands-on/15.mcp/subscriptions) | `subscriptions/listen` 다중 구독 demux, 요청 스코프 알림(진행률·로그) 경로 |
| 5 | [Tasks 실습](#35-tasks-실습) | [15.mcp/tasks](https://github.com/cna-bootcamp/aistudy/tree/main/hands-on/15.mcp/tasks) | 확장 직접 구현, `taskId` + `tasks/get` 폴링 |
| 6 | [AI 에이전트 개발 지원 서비스](#36-ai-에이전트-개발-지원-서비스) | [15.mcp/graphrag](https://github.com/cna-bootcamp/aistudy/tree/main/hands-on/15.mcp/graphrag) | GraphRAG MCP 서버 (조직 지식 연결) |
...
| 파일 | 설명 |
|------|------|
| [server/calc_server.py](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/simple/server/calc_server.py) | 계산기 서버 — Tools + Resources + Prompts |
| [server/weather_server.py](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/simple/server/weather_server.py) | 날씨 서버 — wttr.in 연동, `ctx.protocol_version` 확인 |
| [client/client.py](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/simple/client/client.py) | 계산기 서버 연결 테스트 |
| [client/weather_client.py](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/simple/client/weather_client.py) | 날씨 서버 연결 테스트 |
| [client/README.md](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/simple/client/README.md) | **실행 방법·트러블슈팅** |
...
| 파일 | 설명 |
|------|------|
| [server/travel_server.py](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/mrtr/server/travel_server.py) | 여행 플래너 서버 — `Resolve()`/`Elicit()` 3단계, `RequestStateSecurity` |
| [client/client.py](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/mrtr/client/client.py) | `elicitation_callback` 등록, 라운드 출력 (`--auto`/`--interactive`/`--decline`) |
| [client/README.md](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/mrtr/client/README.md) | **실행 방법·트러블슈팅** |
...
2026-07-28은 알림을 **두 갈래**로 명확히 나눔. 이 절은 두 갈래를 각각 실습함.

| 실습 | 주제 | 스크립트 |
|------|------|----------|
| [3.4.1](#341-변경-알림-구독) | **변경 알림** — `subscriptions/listen` 스트림으로 전달 | `client.py` |
| [3.4.2](#342-요청-스코프-알림-진행률로그) | **요청 스코프 알림** — 진행률·로그는 **그 요청의 응답 스트림**으로 전달 | `request_scoped_client.py` |
...
두 실습이 **같은 서버**를 공유하며 클라이언트 스크립트만 다름.

| 파일 | 실습 | 설명 |
|------|------|------|
| [server/watch_server.py](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/subscriptions/server/watch_server.py) | 공통 | 변경 알림(`notify_*`) + 진행률·로그(`long_task`) 발송 |
| [client/client.py](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/subscriptions/client/client.py) | 3.4.1 | 두 구독을 동시에 열고 demux 확인 |
| [client/request_scoped_client.py](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/subscriptions/client/request_scoped_client.py) | 3.4.2 | 진행률·로그가 구독 스트림으로 오지 않음을 건수로 증명 |
| [client/README.md](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/subscriptions/client/README.md) | 공통 | **실행 방법·트러블슈팅** |
...
> **예제 코드 및 실행 방법**:
> [hands-on/15.mcp/subscriptions/client/README.md](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/subscriptions/client/README.md) 참조
> — 실행 스크립트는 [client/client.py](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/subscriptions/client/client.py)임.
...
### 3.4.2 요청 스코프 알림 (진행률·로그)

**진행률·로그 알림은 구독 스트림이 아니라 해당 요청의 응답 스트림으로 전달됨.**
이 실습은 **구독 스트림을 열어 둔 채로** 진행률·로그를 발생시키는 도구를 호출하여,
구독 스트림에 아무것도 오지 않는다는 것을 **건수로 증명**함.

> **예제 코드 및 실행 방법**:
> [hands-on/15.mcp/subscriptions/client/README.md](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/subscriptions/client/README.md#45-요청-스코프-알림-request_scoped_clientpy) 4.5절 참조
> — 실행 스크립트는 [client/request_scoped_client.py](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/subscriptions/client/request_scoped_client.py),
> 서버는 [server/watch_server.py](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/subscriptions/server/watch_server.py)의 `long_task` 도구임.

#### 학습 포인트

| 항목 | 내용 |
|------|------|
| **전달 경로 분리** | `notifications/progress`·`notifications/message`는 **그 요청의 응답 스트림**으로만 흐름. `subscriptions/listen` 스트림에는 실리지 않음 |
| **진행률 옵트인** | 요청 `_meta`에 `progressToken`이 있어야 함. SDK는 `progress_callback`을 주면 자동으로 실어 보냄 |
| **로그 옵트인** | 요청 `_meta`에 `io.modelcontextprotocol/logLevel`이 있어야 함. 서버는 **없는 요청에 `notifications/message`를 보내면 안 됨** |
| **요청별 로그 레벨** | 2026-07-28에서 `logging/setLevel` RPC가 **제거**되고 요청별 `_meta`로 대체됨 |
| **HTTP에서의 의미** | Streamable HTTP에서는 각 요청이 자기 SSE 응답 스트림을 가짐. 진행률·로그는 그 스트림에만 실림 |

> **참고**: Logging 기능 자체는 2026-07-28에서 **폐기 예정**(SEP-2577)임.
> 이 실습은 "알림이 어느 스트림으로 가는가"를 보이기 위해 사용하며,
> 신규 구현은 stdio는 `stderr`, 관측성은 OpenTelemetry를 쓸 것.
...
async def long_task(steps: int = 4, delay: float = 0.3, ctx: Context = None) -> str:
    """단계마다 진행률과 로그를 보고함. 둘 다 이 요청의 응답 스트림으로만 흐름."""
...
async with Client(transport, log_level="info", logging_callback=on_log) as client:
    async with client.listen(tools_list_changed=True, resources_list_changed=True,
                             resource_subscriptions=["watch://notes"]) as sub:
        # 구독 스트림은 열려 있지만 아래 호출의 진행률·로그는 여기로 오지 않음
...
```
[목표]
MCP 요청 스코프 알림(진행률·로그) 예제 개발
 - 진행률·로그 알림은 구독 스트림이 아니라 해당 요청의 응답 스트림으로 전달됨을 증명
[역할]
당신은 MCP에 능통한 AI앱 개발자임
[입력]
- MCP교재: agentic-ai/textbook/15.MCP.md
[처리]
- MCP교재의 2.4.3 절 '어떤 알림이 어디로 가나' 표와 2.2.5 전송 방식 절 이해
- 예제 개발
  - 비즈니스 시나리오
    - 업무명: 알림 전달 경로 검증
    - 흐름:
      - 클라이언트가 모든 변경 종류를 구독하는 subscriptions/listen 스트림을 먼저 열어둠
      - 진행률과 로그를 발생시키는 오래 걸리는 도구를 호출
      - 진행률·로그는 그 요청의 응답 스트림으로 수신되고,
        구독 스트림 수신 건수는 0이어야 함
      - 실제 변경(메모 추가)을 일으키면 그때는 구독 스트림이 반응해야 함
    - 검증 시나리오 4종
      - A: progress_callback O + log_level O -> 진행률·로그 수신, 구독 0건
      - B: progress_callback X + log_level O -> 진행률 0건, 로그는 수신
      - C: 실제 변경 발생 -> 구독 스트림 수신
      - D: log_level 미지정 클라이언트 -> 로그 0건
  - 기술 요구사항
    - {base}: `hands-on/15.mcp`
    - {프로젝트}: subscriptions (3.4.1 예제와 같은 서버를 공유)
    - 파일명: client/request_scoped_client.py
    - MCP SDK: 최신 버전 (`mcp[cli]>=2.0.0`, 2026-07-28 스펙)
    - 서버 구현 (기존 watch_server.py에 도구 추가)
      - long_task(steps, delay, ctx): 단계마다
        ctx.report_progress(i, steps, message)와 ctx.info({...}) 호출
      - Logging은 폐기 예정 기능이므로 MCPDeprecationWarning이 발생함.
        예제 목적상 의도적으로 사용하되 경고는 끄고 그 이유를 주석으로 남길 것
    - 클라이언트 구현
      - Client(transport, log_level="info", logging_callback=...)로 로그 옵트인
      - call_tool(..., progress_callback=...)로 진행률 옵트인
      - 모든 변경 종류를 구독하는 listen 스트림을 백그라운드 태스크로 열어두고 건수 집계
      - 각 시나리오마다 '수신 건수'와 '기대값'을 함께 출력하여 검증이 눈에 보이게 할 것
      - 최종 집계표 출력
    - STDIO 전송. 서버 로그는 stderr로만 출력
  - 의존성 정의
- 테스트 및 버그 Fix
  - A~D 시나리오의 기대 건수가 모두 맞는지 확인
- README.md 작성 (기존 subscriptions README에 절 추가)
  - 두 갈래 알림 경로 다이어그램
  - 진행률·로그의 옵트인 방법 표
  - 소스 코드 설명
  - 실행 방법 및 실제 실행 결과
[출력]
- `{base}/{프로젝트}/server/watch_server.py` (long_task 도구 추가)
- `{base}/{프로젝트}/client/request_scoped_client.py` (신규)
- `{base}/{프로젝트}/client/README.md` (절 추가)
```

> **심화 과제(선택)**: 서버에서 `logLevel`이 없는 요청에도 `ctx.info()`를 호출하면
> 어떻게 되는지 확인할 것(스펙상 서버가 보내면 안 되며, SDK가 걸러 줌).
> Streamable HTTP로 전송을 바꿔 각 요청이 자기 SSE 응답 스트림을 갖는 것도 관찰해 볼 것.
...
| 파일 | 설명 |
|------|------|
| [common/task_protocol.py](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/tasks/common/task_protocol.py) | 서버·클라이언트 공용 와이어 타입 (`TaskCreatedResult` 등) |
| [server/tasks_extension.py](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/tasks/server/tasks_extension.py) | 서버측 확장 — `intercept_tool_call`, `MethodBinding` |
| [server/batch_server.py](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/tasks/server/batch_server.py) | 배치 도구 + 확장 등록 |
| [client/tasks_client_extension.py](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/tasks/client/tasks_client_extension.py) | 클라이언트측 확장 — `ResultClaim` 폴링 |
| [client/client.py](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/tasks/client/client.py) | 완료/실패/취소/대조 4개 시나리오 |
| [client/README.md](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/tasks/client/README.md) | **실행 방법·트러블슈팅** |
...
> **예제 코드 및 실행 방법**:
> [hands-on/15.mcp/graphrag/README.md](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/graphrag/README.md) 참조
> — 서버는 [graphrag/server.py](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/graphrag/server.py),
> 검색 로직은 [graphrag/search_service.py](https://github.com/cna-bootcamp/aistudy/blob/main/hands-on/15.mcp/graphrag/search_service.py)임.
>
> **주의**: 이 예제는 아직 **v1(`FastMCP`) 기준**이라 최신 SDK로는 실행되지 않음.
> 아래 프롬프트로 새로 생성하거나 [2.9 v1 → v2 대응표](#29-생태계-현황)로 변환할 것.
