...
> **교육 관점 유의**: Sampling·Roots는 본 교재의 실습(3.3, 3.5)에서 여전히 다루지만, **신규 구현에는 채택하지 않는 것이 스펙 권고**임.
> 실습의 목적은 "역방향 요청이라는 개념"과 "MRTR로 바뀐 방식"을 이해하는 데 있음.
...
async def main():
    # stdio 서브프로세스, HTTP URL, 인메모리 서버 객체를 모두 수용
...
async with Client("server.py", elicitation_callback=on_elicit) as client:
...
>
> **주의**: `hands-on/15.mcp/` 아래에 이미 배포된 예제 소스는 이전 v1(`FastMCP`) 기준으로 작성되어 있음.
> 최신 SDK를 설치하면 그대로 실행되지 않으므로, 각 실습의 프롬프트로 **새로 생성**하거나
...
> **참고**: 기존에 배포된 `hands-on/15.mcp/simple` 소스는 v1(`FastMCP`) 기준으로 작성되어 있음.
> 위 프롬프트로 새로 생성하거나, [2.9 v1 → v2 대응표](#29-생태계-현황)를 참고해 변환할 것.
...
> **⚠️ Deprecated 기능**: Sampling은 2026-07-28에서 폐기 예정으로 선언됨(SEP-2577).
> 본 실습의 목적은 **"서버가 클라이언트의 LLM을 빌린다"는 개념**과
> **MRTR로 바뀐 전달 방식**을 이해하는 데 있음. 신규 서비스에는 서버가 LLM 제공자 API와 직접 연동할 것.
...
- 예제 개발
  ...
  - 기술 요구사항
    ...
    - MCP SDK: 최신 버전 (`mcp[cli]>=2.0.0`, 2026-07-28 스펙)
      ...
      - 클라이언트: `from mcp import Client` (ClientSession/initialize() 금지)
      - Sampling은 2026-07-28 스펙에서 MRTR로 동작함
        (서버가 InputRequiredResult 반환 -> 클라이언트가 inputResponses를 실어 재요청)
      - 클라이언트는 Client(server, sampling_callback=...)로 콜백만 등록하면 되고 MRTR 왕복은 SDK가 처리함
      - Sampling은 SEP-2577로 폐기 예정 기능이므로 deprecation 경고가 발생할 수 있음(정상)
...
> **2026-07-28 관점 포인트**: 이 시나리오는 **MRTR의 교과서적 사례**임.
> 3단계 입력 요청은 스펙상 `InputRequiredResult`를 **3번 반환**하고 클라이언트가 **3번 재요청**하는 흐름이 됨.
> 서버는 지금까지 모은 정보를 `requestState`에 **무결성 보호하여 인코딩**함으로써 세션 없이도 다단계 대화를 완성함.
...
- 예제 개발
  ...
  - 기술 요구사항
    ...
    - MCP SDK: 최신 버전 (`mcp[cli]>=2.0.0`, 2026-07-28 스펙)
      - 서버: `from mcp.server.mcpserver import MCPServer` (FastMCP 금지)
      - 클라이언트: `from mcp import Client` (ClientSession/initialize() 금지)
      - Elicitation은 2026-07-28 스펙에서 MRTR로 동작함
        (3단계 입력 요청 = InputRequiredResult 3회 + 클라이언트 재요청 3회)
      - 서버는 mcp.server.mcpserver의 Resolve()/Elicit() 의존성 주입으로 구현할 것
        (Resolve()로 선언한 파라미터는 도구의 input schema에 노출되지 않아 LLM이 값을 지어낼 수 없음)
      - 클라이언트는 Client(server, elicitation_callback=...)로 콜백 등록
      - 단계 간 상태는 세션이 아니라 requestState로 전달하고, 무결성 보호(HMAC 등)를 적용할 것
    - LLM: Groq LPU의 OpenAI 호환 API로 연결
      - Groq API 키: hands-on/.env 파일의 GROQ_API_KEY 사용
      - Groq Base URL: https://api.groq.com/openai/v1
...
> **참고**: 기존에 배포된 `hands-on/15.mcp/elicitation` 소스는 v1(`FastMCP` + 콜백) 기준임.
> 위 프롬프트로 새로 생성하거나 v2로 변환할 것.

> **심화 과제(선택)**: 서버 로그에서 `resultType: "input_required"`가 3회 발생하고
> 각 재요청의 JSON-RPC id가 서로 다른지 확인할 것. `requestState`를 임의로 변조했을 때
> 서버가 이를 거부하는지도 검증해볼 것.
...
> **⚠️ Deprecated 기능**: Roots는 2026-07-28에서 폐기 예정으로 선언됨(SEP-2577).
> 본 실습의 목적은 **"권고 수준의 안전장치는 보안이 아니다"** 라는 교훈을 체득하는 데 있음.
> 신규 구현에서는 접근 범위를 **도구 파라미터·리소스 URI·서버 설정**으로 전달하고,
> 실제 통제는 **OS 권한·컨테이너 격리**로 수행할 것.
...
- 예제 개발
  ...
  - 기술 요구사항
    - {base}: `hands-on/15.mcp`
    - {프로젝트}: roots
    - MCP SDK: 최신 버전 (`mcp>=2.0.0`, 2026-07-28 스펙)
      - 서버: `from mcp.server.mcpserver import MCPServer` (FastMCP 금지)
      - 클라이언트: `from mcp import Client` (ClientSession/initialize() 금지)
      - Roots도 2026-07-28 스펙에서 MRTR로 동작함
        (서버가 InputRequiredResult에 roots/list를 담아 반환 -> 클라이언트가 재요청)
      - 클라이언트는 Client(server, list_roots_callback=...)로 콜백 등록
      - Roots는 SEP-2577로 폐기 예정 기능이므로 deprecation 경고가 발생할 수 있음(정상)
      - 비교 학습: 같은 통제를 "허용 루트를 도구 파라미터로 전달"하는 방식으로도 구현하여
        Roots 없이도 동일 효과를 얻을 수 있음을 보일 것
  - 의존성 정의
- 테스트 및 버그 Fix
- README.md 작성
  - 아키텍처 다이어그램
