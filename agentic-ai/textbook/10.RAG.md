...
| Self-RAG | 1~2회 | 중간 | 검색 없이 LLM 자체 지식으로 충분할 때 |
| CRAG | 2~3회 | 중간 | 검색 품질 불안정 |
...
| **Answer Relevance** | 답변 적절성 | 답변이 사용자의 질문에 직접적으로 대응하는가 |
| **Context Precision** | 검색 정밀도 | 질문과 관련된 문서가 상위 순위에 노출되었는가 |
| **Context Recall** | 검색 재현율 | 찾아야 할 문서 중 실제 찾은 문서의 비율 |
