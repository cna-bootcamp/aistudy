...
- 예제 개발
  - 워크플로우 
    - (큰 흐름) 질문 받기 → 검색방법 선택 → 검색 실행 → 근거가 충분한지 점검
    - 이 MAS는 SAS 패턴으로 구성(§2.1) — 조율자(Scheduler)·실행자(Agent)·감독자(Supervisor)가 역할 분담
    - ① 검색방법 선택  [Scheduler = 일을 나누고 순서를 정하는 '조율자']
      - 질문 성격에 따라 아래 두 검색방법 중 하나를 고름 (이 고르기를 '라우팅'이라 함)
        - vector  (조문 벡터 RAG, ChromaDB patent_law): 조문 원문을 그대로 정밀 인용해야 할 때
        - graphrag(MS GraphRAG): 요건·권리·절차가 서로 어떻게 연결되는지 '관계·커뮤니티(주제 묶음) 구조'를 물을 때
          (Local=특정 항목 주변 탐색 / Global=전체 요약 / DRIFT=Local+Global 결합)
      - 고르는 방법: 먼저 키워드 규칙으로 빠르게 판단(패턴매칭), 애매하면 LLM이 판단(LLM 폴백)
    - ② 검색 실행  [Agent = 실제로 검색을 수행하는 '실행자']
      - 고른 방법으로 검색하고, 두 결과가 함께 나오면 하나로 합쳐 정리(결과 융합)
    - ③ 근거가 충분한지 점검  [Supervisor = 결과를 지켜보다 부족하면 개입하는 '감독자']
      - 찾은 근거가 답변에 충분한지 평가 → 미흡하면 검색방법을 바꿔 다시 검색(재검색)
  - 노출 (MCP 서버)
...
- 예제 개발
  - 워크플로우 (하위 SAS: 내부 StateGraph, Self-RAG)
    - (큰 흐름) 검색이 필요한지 판단 → 소스 골라 검색 → 자료 평가 → 답 생성 → 스스로 점검(부족하면 다시)
    - Self-RAG란? LLM이 검색·생성을 하며 스스로 품질을 점검(IsRel/IsSup/IsUse)하고 부족하면 재검색하는 RAG 방식
    - ① 검색 경로 결정  [Route = 어디서 찾을지 길 정하기]
      - 먼저 '검색이 필요한 질문인가'를 판단하고, 필요하면 어느 소스에서 찾을지 고름 (law / web / youtube)
    - ② 소스별 검색  [Retrieve = 실제로 찾아오기]
      - 고른 소스에 맞게 검색어를 다듬은(쿼리 최적화) 뒤 검색
        - law (korean-law MCP): 판례 검색(search_decisions) · 최신 법령(search_law/get_law_text) · 종합검색(chain_full_research)
        - web (DuckDuckGo): 뉴스·시장 동향
        - youtube (YouTube API v3 + YouTubeLoader): 강의·해설 자료. 자막은 YouTubeLoader로 로드 ('YouTube 서치 예제' 참고)
        - (참고) korean-law MCP 키는 hands-on/.env의 'LAW_OC' 값 사용
    - ③ 스스로 점검하며 답 생성  [Self-RAG 3단계 자가 점검]
      - [IsRel] 찾은 자료가 질문과 관련 있는지(Relevant) 한 번에 평가 → 관련된 것만 사용
      - [생성] 관련 자료를 바탕으로 답변 생성
      - [IsSup] 답변이 자료로 뒷받침되는지(Supported, 근거성) 점검
      - [IsUse] 답변이 실제로 쓸모 있는지(Useful) 점검 → 부족하면 검색어를 새로 써서 다시 검색(재검색)
  - 통신
...
- 예제 분석 및 이해
- 예제 개발
  - 워크플로우 — 런타임 인라인 게이트 (답을 만드는 도중 단계마다 끼워둔 '검문소'. 매 요청 동작 / 정답 없이 평가=reference-free / 빠름=저지연)
    - (큰 흐름) 초안 작성 → 근거 검증 → 인용 검증 → 사람 승인 → 개인정보 가리고 최종 출력
    - ① 작성 — MAS A(법령)·MAS B(동향)가 찾아온 자료를 종합해 의견서 초안 생성
    - ② 근거성 검증  [검문소 1] — 초안이 찾은 자료에 실제로 근거하는지 점검
      - Self-RAG의 [IsSup](근거성) 평가기(grader) 사용 — 찾은 자료만으로 판단, 정답 불필요, LLM 1번 호출
    - ③ 인용 검증  [검문소 2] — 법령·판례 인용이 진짜인지(지어낸 '환각'인지) 확인
      - korean-law MCP의 verify_citations로 검증 → 가짜 인용이면 다시 작성(재작성)
    - ④ 사람 승인  [검문소 3, HITL = 사람이 직접 확인하는 단계]
      - 법적 책임이 큰 출력이라 확정 전 사람이 검토·승인 (LangGraph interrupt_before로 그 직전에 잠시 멈춤)
    - ⑤ 개인정보 가리기  [DLP = 민감정보 마스킹] — 개인정보를 가린 뒤 최종 출력
    - (주의) RAGAS는 이 실시간 검문소로 쓰지 않음 — 아래 '오프라인 품질 검증' 참고
...
- 예제 개발
  - 워크플로우 (상위 SAS: Orchestrator StateGraph)
    - (큰 흐름) 질문 의도 분류 → 알맞은 MAS로 보냄 → (필요 시 A·B 동시 실행) → 결과 모아 C로 → 최종 답
    - 이 Orchestrator는 '상위 SAS' — 단위 MAS A/B/C(하위 SAS)를 거느리고 조율하는 윗 계층임 (2계층 구조, §8.2)
    - ① 의도 분류 후 알맞은 MAS로 보내기  [Scheduler]
      - 질문 의도를 분류해 담당 MAS로 보냄(=라우팅). 먼저 키워드 규칙(패턴매칭), 애매하면 LLM 판단(LLM 폴백)
        - 법령 개념·요건·조문   → MAS A 단독
        - 선행기술·판례·동향    → MAS B (법령 근거가 필요하면 MAS A도 동시에 함께 돌려 보강)
        - 의견서·서면 작성 요청 → A와 B를 동시에(∥) 돌린 뒤 → 그 결과를 MAS C가 모아 작성
    - ② 호출 순서 — 동시에 펼치고(fan-out) → 한곳에 모으기(fan-in)   ※개념은 교재 §1.2·§7.7 참고
      - 누가 누구에 의존? A·B는 '질문'만 있으면 되어 서로 독립 / C는 A·B 결과가 있어야 하므로 A·B에 의존
      - 동시 실행(fan-out): 라우터가 필요한 MAS(A·B)에 일감을 동시에 뿌림 (조건부 선택 분기는 LangGraph Send API 사용)
      - 모아 실행(fan-in): C는 A·B 결과가 모두 도착(합류)한 뒤 딱 1번 실행
      - 기본은 A·B 동시 실행 권장(독립이라 순서대로 하면 시간만 2배). 단, B가 A 결론을 받아 검색을 좁히는 설계면 A→B 순차도 가능
    - ③ 각 MAS 실제 호출  [Agent]
      - A는 MCP(Streamable HTTP)로, B·C는 API/Function Calling으로 호출
      - 동시 실행 구간은 asyncio.gather로 네트워크 대기를 겹쳐 시간 단축, 갈래마다 asyncio.wait_for로 시간 제한(타임아웃)
    - ④ 전체 감시·통제  [Supervisor(글로벌) = 모든 갈래를 위에서 지켜보는 총감독]
      - 예산(Budget)을 갈래별로 나눠 배정 · 무한루프 방지(Loop Guard) · 출력 검증 · 위험하면 사람 승인으로 올림(HITL 승급)
      - 일부 갈래가 실패해도 나머지로 최대한 답함 (graceful degradation = 부분 실패해도 우아하게 버티기)
    - ⑤ 결과 합치기 — 병렬 결과는 Shared State에 모아 합침(reducer=operator.add로 충돌 없이 병합), 대화 맥락은 checkpointer로 멀티턴 유지
  - 통신
