...
이 페이지는 **pgvector** 번들에서 사용 가능한 컴포넌트를 설명함.

## pgvector 벡터 스토어

**PGVector** 컴포넌트는 `PGVector` 인스턴스를 사용하여 PostgreSQL 벡터 스토어에 읽기/쓰기 수행.
...
허용되는 값 및 기능에 대한 정보는 PGVector 문서를 참조하거나 컴포넌트 코드를 검사함.

| 이름 | 타입 | 설명 |
|------|------|------|
| pg_server_url | SecretString | 입력 파라미터. PostgreSQL 서버 연결 문자열 |
| collection_name | String | 입력 파라미터. 벡터 스토어의 테이블 이름 |
