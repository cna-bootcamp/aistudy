...
MCP 서버가 기존 14.graphrag 인덱싱 결과를 읽기 위해 `Neo4jGraph`를 생성하고,
스키마·통계·벡터 인덱스 차원 검증 정보를 제공함. (Neo4j retrieve 예제와 동일 로직)
"""
import logging
import time
...
class Neo4jConnection:
    ...
    def _connect_with_retry(self, max_retries: int = 3) -> Neo4jGraph:
        """Neo4j 연결을 최대 3회 재시도 (지수 백오프)."""
        for attempt in range(max_retries):
            try:
                # Neo4jGraph: LangChain Neo4j 래퍼. sanitize=True는 쿼리 결과에서 임베딩 같은
                # 대형 리스트 속성을 잘라내 LLM 컨텍스트/응답 직렬화를 가볍게 유지함.
                graph = Neo4jGraph(
    ...
    def validate_vector_dimensions(self) -> list[str]:
        """벡터 인덱스 차원이 설정 임베딩 차원(4096)과 일치하는지 확인 후 경고 목록 반환."""
        warnings: list[str] = []
