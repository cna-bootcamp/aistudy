...
| 항목 | 내용 |
|------|------|
| 실행 위치 | `hands-on/02.multiturn/full-history/` |
...
```text
hands-on/02.multiturn/full-history/
...
매 API 호출 시 `system` 메시지 + 이전 모든 `user`/`assistant` 메시지를 포함한  
전체 `messages` 리스트를 전송함. 대화가 길어질수록 토큰 사용량이 증가하는 특징이 있음.

```python
messages = [{"role": "system", "content": SYSTEM_PROMPT}]

# 매 턴마다 전체 히스토리를 그대로 API에 전송
response = client.chat.completions.create(
    model="gpt-4o-mini",
    messages=messages,   # system + 이전 모든 user/assistant 포함
)
```
