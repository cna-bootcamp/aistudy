 ...
 * 여는 법: explain-exam/index.html?data=../09.langchain/gemini/explain/data.js
 *
...
window.EXPLAIN_DATA = {
  meta: {
    title: "여행 플래너 (LangChain + Gemini) 예제 설명",
    entry: "travel_planner.py",
  },

  files: [
    { id: "main",    label: "travel_planner.py",  role: "메인 파일 · 화면과 대화 흐름 (Gemini 버전)" },
  ...
  flow: [
    ...
    { step: 5, title: "메시지 변환", label: "메시지 변환", refs: ["build_messages"],
    ...
    { step: 6, title: "AI 에이전트 실행", label: "에이전트 실행", refs: ["get_agent", "generate_response"],
      summary: "get_agent()가 만든 ReAct 에이전트가 invoke() 한 번으로 '생각→도구 호출→관찰' 루프를 자동 수행",
      detail: "주방장(AI)에게 주문서를 넘기는 핵심 단계임. Claude 버전과 달리 ChatGoogleGenerativeAI(Gemini 모델)를 사용하지만, create_agent가 이 반복(ReAct 루프)을 라이브러리가 알아서 처리하는 방식은 동일함. 개발자는 invoke() 한 번만 부르면 됨." },
    ...
    { step: 8, title: "응답 생성", label: "응답 생성", refs: ["generate_response"],
      summary: "도구 결과를 종합해 최종 답변(AIMessage)을 만들고 텍스트를 추출함",
      detail: "재료로 요리를 완성하는 단계임. Gemini는 답변을 단일 문자열로 반환하므로 Claude 버전처럼 별도의 블록 분리 처리가 필요 없음. generate_response()가 content를 그대로 반환함." },
    { step: 9, title: "결과 표시", label: "결과 표시", refs: ["display_sidebar", "extract_tool_trace"],
  ...
  functions: [
    // ===== travel_planner.py (메인) =====
    {
      ...
      lines: [
        { at: 'if "messages" not in st.session_state:  # st.session_state', text: "st.session_state는 '탭이 열려 있는 동안 유지되는 메모장'임. messages(대화 내용)라는 항목이 아직 없을 때만 빈 목록으로 만듦." },
        { at: 'if "agent" not in st.session_state:', text: "agent가 아직 없을 때만 None으로 초기화함. 처음 실행 시에만 에이전트를 새로 만들게 됨." },
...
`def initialize_session_state() -> None:
    ...
    },
    {
      ...
      name: "get_agent()",
      fileId: "main",
      summary: "Gemini 모델과 도구들을 묶어 'ReAct 에이전트'를 한 번만 만들어 두고, 이후에는 저장해 둔 것을 재사용함.",
      how: "에이전트를 만드는 일은 비용이 들기 때문에, 매번 만들지 않고 처음 한 번만 만들어 st.session_state.agent에 보관함(이를 '지연 생성 + 캐싱'이라 함). Claude 버전과의 차이점: ChatAnthropic 대신 ChatGoogleGenerativeAI를 쓰고 api_key 대신 google_api_key 파라미터를 사용함. create_agent(llm, TRAVEL_TOOLS)가 ReAct 루프를 자동 처리하는 것은 동일함.",
      terms: ["지연 생성(lazy)", "캐싱(cache)", "LangChain", "LangGraph", "ChatGoogleGenerativeAI", "create_agent", "ReAct 루프", "temperature"],
      lines: [
        { at: 'if st.session_state.agent is None:', text: "agent가 아직 없을(None) 때만 새로 만듦. 이미 있으면 이 블록을 건너뛰고 저장된 것을 그대로 씀(캐싱)." },
        { at: 'api_key = require_api_key("GEMINI_API_KEY")', text: "require_api_key()로 Gemini API 키를 가져옴. Claude 버전이 CLAUDE_API_KEY를 쓰는 것과 달리 GEMINI_API_KEY를 사용함." },
        { at: 'llm = ChatGoogleGenerativeAI(', text: "ChatGoogleGenerativeAI는 Google Gemini 모델을 LangChain에서 쓰기 쉽게 감싼 객체임. google_api_key 파라미터 이름이 Claude 버전의 api_key와 다름." },
...
`def get_agent():
    ...
    if st.session_state.agent is None:
        ...
        # ChatGoogleGenerativeAI: LangChain Google Generative AI 채팅 모델 래퍼 (llm.invoke()로 대화 요청 전송)
    ...
    },
    {
      ...
      how: "AI에게 보낼 '주문서'를 정리하는 함수임. 맨 앞에 시스템 지침(SYSTEM_PROMPT)을 SystemMessage로 넣고, 그동안의 대화를 역할에 따라 HumanMessage(사람)·AIMessage(AI)로 바꿔 담음. Gemini는 대화 시작이 HumanMessage여야 하므로 SystemMessage를 첫 번째로 배치함. 비용을 아끼려고 최근 10개만 포함하고, 마지막에 이번 새 입력을 붙임.",
      terms: ["SystemMessage", "HumanMessage", "AIMessage", "리스트(list)", "타입 힌트"],
      lines: [
        { at: 'messages = [SystemMessage(content=SYSTEM_PROMPT)]', text: "메시지 목록의 맨 앞에 시스템 지침을 넣음. Gemini가 대화 맥락을 올바르게 이해하도록 가장 먼저 배치함." },
...
`def build_messages(user_input: str) -> list:
    ...
    ChatGoogleGenerativeAI는 대화 시작이 HumanMessage여야 하므로
    SystemMessage를 첫 번째로 배치하고 이력을 그 뒤에 추가함.
    """
    ...
    },
    {
      ...
      terms: ["isinstance()", "tool_calls", "ToolMessage", "tool_call_id", "AIMessage", "JSON", "예외 처리(try/except)", "딕셔너리(dict)"],
      lines: [
...
`def extract_tool_trace(messages: list) -> list[dict[str, Any]]:
    ...
    },
    {
      ...
      how: "이 함수가 전체를 지휘함: ①에이전트 준비 → ②메시지 정리 → ③invoke()로 한 번에 실행(도구 호출 루프 포함) → ④도구 기록 추출 → ⑤마지막 메시지에서 텍스트 추출. Claude 버전과 달리 Gemini는 답을 단일 문자열로 반환하므로 다중 콘텐츠 블록 처리 코드가 없어 더 단순함.",
      terms: ["invoke()", "create_agent", "ReAct 루프", "isinstance()", "AIMessage"],
      lines: [
        ...
        { at: 'content = output_msgs[-1].content', text: "결과 메시지들 중 맨 마지막([-1])이 AI의 최종 답변임. Gemini는 단일 문자열을 반환하므로 그대로 사용함." },
        { at: 'return content or "응답을 생성할 수 없습니다."', text: "content가 비어있으면 기본 오류 문구를 돌려줌." },
      ],
      code:
...
`def display_sidebar() -> None:
    ...
    },
    {
      ...
      how: "프로그램의 '시작 버튼'에 해당함. 페이지 설정 → 상태 초기화 → 사이드바·환영문·이전 대화 표시 → 채팅창 입력 처리 순으로 진행함. Claude 버전과의 차이: 페이지 캡션이 'Gemini'임. 사용자가 입력하면(:= 로 받음) 답변을 만들어 말풍선으로 보여주고, 오류가 나도 멈추지 않게 try/except로 감쌈.",
      terms: ["st.chat_input", "st.chat_message", "st.empty", ":= (바다코끼리)", "예외 처리(try/except)", "Streamlit"],
      lines: [
        { at: 'page_title=f"{APP_TITLE} - Gemini"', text: "Claude 버전과 달리 페이지 제목에 'Gemini'가 표시됨. 어떤 모델을 사용하는 예제인지 구분할 수 있음." },
        { at: 'st.caption("LangChain ChatGoogleGenerativeAI + create_agent + Streamlit")', text: "화면 부제목에 사용한 LangChain 모델 래퍼(ChatGoogleGenerativeAI)가 표시됨." },
...
`def main() -> None:
    ...
    st.caption("LangChain ChatGoogleGenerativeAI + create_agent + Streamlit")
...
`def normalize_city_name(city: str) -> str:
    ...
    },
    {
      ...
      lines: [
        ...
        { at: 'return f"https://www.google.com/maps/search/?api=1&query={query}"', text: "f-string으로 구글 지도 검색 URL을 완성해 돌려줌." },
      ],
      code:
...
def get_weather(city: str) -> dict:
    ...
    },
    {
      ...
      lines: [
        { at: '"""Search top tourist attractions in a city', text: "@tool로 등록된 관광지 검색 도구임. 이 영문 docstring을 AI가 읽고 언제 호출할지 판단함." },
        ...
        { at: 'return {"error": f"Google Places 관광지 검색 오류', text: "오류가 나면 error 항목을 담아 반환(앱이 멈추지 않음)." },
      ],
      code:
`@tool
...
def get_restaurants(
    ...
    // ===== common/prompts.py (지침서) =====
    {
      ...
      how: "코드가 아니라 'AI에게 주는 규칙을 적은 긴 문장'임. 요청 유형 판단법, 도시명 영문 변환 규칙, 날씨에 따른 추천 기준, 장소 표기 형식 등을 자연어로 적어 둠. build_messages()에서 SystemMessage로 맨 앞에 넣어 매 대화에 적용함. Claude·Gemini 버전이 동일한 파일을 공유하므로 모델 교체 시 프롬프트를 새로 만들 필요가 없음.",
      ...
      lines: [
        ...
        { at: '3. 함수 호출 시 city 값은 반드시 영문', text: "도시명은 반드시 영문으로 넘기라는 규칙 등, AI의 행동을 자연어로 안내함." },
...
`def require_api_key(env_name: str) -> str:
    ...
    // ===== common/ui_text.py (화면 문구) =====
    {
      ...
      how: "코드 곳곳에 문구를 흩어 두면 수정이 번거로움. 자주 바뀌는 안내 문구를 상수(대문자 이름)로 모아두면 한 곳만 고쳐도 전체에 반영됨. Claude·Gemini·OpenAI 버전이 동일한 파일을 공유하므로, 모델이 달라도 같은 UI 문구를 씀.",
      terms: [],
      lines: [
...
"""`,
  ...
  glossary: {
    ...
    "ChatGoogleGenerativeAI": "Google의 Gemini 모델을 LangChain에서 쓰기 쉽게 감싼 객체. Claude 버전의 ChatAnthropic과 같은 역할. google_api_key 파라미터로 인증함.",
