 ...
 * 여는 법: explain-exam/index.html?data=../09.langchain/claude/explain/data.js
 *
...
window.EXPLAIN_DATA = {
  meta: {
    title: "여행 플래너 (LangChain + Claude) 예제 설명",
  ...
  flow: [
    ...
    { step: 8, title: "응답 생성", label: "응답 생성", refs: ["generate_response"],
      summary: "도구 결과를 종합해 최종 답변(AIMessage)을 만들고, 텍스트만 깔끔히 추출함",
      detail: "재료로 요리를 완성하는 단계임. AI가 날씨·관광지·맛집 정보를 묶어 하루 일정을 글로 작성함. generate_response()가 답변에서 화면에 표시할 텍스트만 뽑아냄." },
  ...
  functions: [
    // ===== travel_planner.py (메인) =====
    {
      ...
      lines: [
        { at: '# st.session_state:', text: "st.session_state는 '탭이 열려 있는 동안 유지되는 메모장'임. 여기 넣은 값은 화면을 다시 그려도 살아남음." },
        { at: 'if "messages" not in', text: "messages(대화 내용)라는 항목이 아직 없을 때만 빈 목록 []으로 만듦. 이미 있으면 건드리지 않아 기존 대화가 보존됨." },
        { at: 'if "last_tool_trace" not in', text: "last_tool_trace는 '직전에 어떤 도구를 호출했는지' 기록을 담을 빈 목록임." },
...
`def initialize_session_state() -> None:
    ...
    },
    {
      ...
      terms: ["지연 생성(lazy)", "캐싱(cache)", "LangChain", "LangGraph", "ChatAnthropic", "create_agent", "ReAct 루프", "bind_tools", "temperature", "tool_calls", "AIMessage", "ToolMessage"],
      lines: [
        ...
        { at: 'st.session_state.agent = create_agent(', text: "★핵심★ create_agent가 'AI + 도구목록'을 받아, 도구를 호출하고 결과를 다시 판단하는 반복(ReAct 루프)을 자동 처리하는 에이전트를 만들어 저장(캐싱)함." },
...
`def get_agent():
    ...
    },
    {
      ...
      lines: [
        { at: 'messages: list = [SystemMessage(', text: "메시지 목록의 맨 앞에 시스템 지침을 넣음. SystemMessage는 'AI에게 주는 역할·규칙'을 담는 메시지 종류임." },
        { at: 'st.session_state.messages[-10:]', text: "[-10:]은 '뒤에서 10개만' 잘라오는 파이썬 문법임. 대화가 길어질수록 비용이 늘기 때문에 최근 것만 보냄." },
...
`def build_messages(user_input: str) -> list:
    ...
    },
    {
      ...
      lines: [
        ...
        { at: 'result = json.loads(msg.content)', text: "결과 내용을 JSON으로 해석해 'error' 항목이 있으면 실패로 표시함. 형식이 깨져도 멈추지 않도록 try/except로 감쌈." },
      ],
      code:
`def extract_tool_trace(messages: list) -> list[dict[str, Any]]:
    ...
    },
    {
      ...
      how: "이 함수가 전체를 지휘함: ①에이전트 준비 → ②메시지 정리 → ③invoke()로 한 번에 실행(도구 호출 루프 포함) → ④도구 기록 추출 → ⑤마지막 메시지에서 텍스트만 추출. Claude는 답을 여러 조각(블록) 리스트로 줄 때가 있어서, 그 경우 텍스트 조각만 이어 붙임.",
      terms: ["invoke()", "create_agent", "ReAct 루프", "isinstance()", "리스트 컴프리헨션", ".get()", "AIMessage"],
      lines: [
        { at: 'agent = get_agent()', text: "준비된(또는 새로 만든) 에이전트를 가져옴." },
        { at: 'result = agent.invoke(', text: "★핵심★ agent.invoke()를 단 한 번 호출하면, 내부에서 '판단→도구 호출→결과 반영'을 끝날 때까지 자동 반복함. 예전의 수동 반복문이 이 한 줄로 대체됨." },
        { at: 'st.session_state.last_tool_trace = extract_tool_trace(', text: "이번 실행에서 호출된 도구 기록을 뽑아 사이드바용으로 저장함." },
        { at: 'content = output_msgs[-1].content', text: "결과 메시지들 중 맨 마지막([-1])이 AI의 최종 답변임. 그 안의 content를 꺼냄." },
        { at: 'if isinstance(content, list):', text: "Claude는 가끔 답을 여러 조각의 리스트로 줌. 그럴 때 텍스트 조각만 골라 하나의 문자열로 이어 붙임." },
      ],
      code:
`def generate_response(user_input: str) -> str:
    ...
    # AIMessage.content가 리스트인 경우(Anthropic 다중 콘텐츠 블록 형식) 텍스트 블록만 추출하여 이어 붙임
...
`def normalize_city_name(city: str) -> str:
    ...
    },
    {
      ...
      lines: [
        ...
        { at: 'quote_plus(', text: "quote_plus는 공백을 +로 바꾸는 등, 글자를 URL에 안전하게 넣을 수 있게 인코딩함." },
...
`def build_google_maps_search_url(place_name: str, city: str) -> str:
    ...
    },
    {
      ...
      lines: [
        { at: 'requests.request(', text: "requests.request가 실제로 인터넷에 요청을 보냄. timeout=12로 너무 오래 기다리지 않게 함." },
...
`def _search_places(query: str, city: str, max_results: int) -> list[dict[str, Any]]:
    ...
    },
    {
      ...
      lines: [
        ...
        { at: 'except requests.exceptions.HTTPError', text: "통신 오류가 나도 앱이 죽지 않게, error 항목을 담아 정상적으로 반환함." },
      ],
      code:
`@tool
...
`def require_api_key(env_name: str) -> str:
    ...
    // ===== common/ui_text.py (화면 문구) =====
    {
      ...
      lines: [
        ...
        { at: 'TECH_GUIDE =', text: "TECH_GUIDE: ReAct 동작 흐름을 간단히 정리한 기술 안내." },
      ],
      code:
`APP_TITLE = "여행 플래너"
APP_ICON = "🗺️"
