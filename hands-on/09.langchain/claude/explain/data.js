...
`def initialize_session_state() -> None:
    ...
    },
    {
      ...
      terms: ["지연 생성(lazy)", "캐싱(cache)", "LangChain", "LangGraph", "ChatAnthropic", "create_react_agent", "ReAct 루프", "bind_tools", "temperature", "tool_calls", "AIMessage", "ToolMessage"],
      lines: [
        ...
        { at: 'st.session_state.agent = create_react_agent(', text: "★핵심★ create_react_agent가 'AI + 도구목록'을 받아, 도구를 호출하고 결과를 다시 판단하는 반복(ReAct 루프)을 자동 처리하는 에이전트를 만들어 저장(캐싱)함." },
...
`def extract_tool_trace(messages: list) -> list[dict[str, Any]]:
    ...
    },
    {
      ...
      terms: ["invoke()", "create_react_agent", "ReAct 루프", "isinstance()", "리스트 컴프리헨션", ".get()", "AIMessage"],
      lines: [
