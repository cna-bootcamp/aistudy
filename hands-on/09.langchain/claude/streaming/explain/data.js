 ...
 * 여는 법: explain-exam/index.html?data=../09.langchain/claude/streaming/explain/data.js
 *
...
window.EXPLAIN_DATA = {
  meta: {
    title: "여행 플래너 (LangChain + Claude Streaming) 예제 설명",
    entry: "travel_planner.py",
  },

  files: [
    { id: "main",    label: "travel_planner.py",  role: "메인 파일 · 화면과 스트리밍 대화 흐름" },
  ...
  flow: [
    { step: 1, title: "앱 시작", label: "앱 시작", refs: ["initialize_session_state"],
      summary: "main()이 실행되어 화면 제목·아이콘을 정하고, 기억 공간을 준비함",
    ...
    { step: 2, title: "에이전트 준비 (캐싱)", label: "에이전트 준비", refs: ["get_agent"],
    ...
    { step: 3, title: "화면 구성", label: "화면 구성", refs: ["display_sidebar", "display_chat_history"],
    ...
    { step: 4, title: "사용자 입력 대기", label: "사용자 입력 대기",
      summary: "화면 맨 아래 채팅창(st.chat_input)에서 입력을 기다림",
      detail: "손님의 주문을 기다리는 단계임. 사용자가 도시명이나 질문을 입력하고 Enter를 누르면 다음 단계로 넘어감. 입력이 없으면 여기서 계속 대기함." },
    { step: 5, title: "입력 저장·표시", label: "입력 저장·표시",
    ...
    { step: 6, title: "대화 이력 변환", label: "대화 이력 변환", refs: ["build_history"],
      summary: "build_history()가 지금까지의 대화를 LangChain 메시지 객체 목록으로 변환함",
      detail: "AI가 알아듣는 형식으로 이전 대화를 정리하는 단계임. 사람 말은 HumanMessage, AI 말은 AIMessage 객체로 변환하고, 비용 절약을 위해 최근 10개만 포함함. 시스템 지침(SYSTEM_PROMPT)은 이미 get_agent() 안에 넣었으므로 여기서는 빼도 됨." },
    { step: 7, title: "스트리밍 응답 생성", label: "스트리밍 응답 생성", refs: ["stream_response"],
      summary: "stream_response()가 agent.stream()을 호출해 텍스트 청크를 실시간으로 yield함",
      detail: "요리를 접시에 완성된 뒤 한꺼번에 내는 게 아니라, 재료를 다듬으면서 실시간으로 접시에 올려주는 단계임. agent.stream()이 ReAct 루프(생각→도구 호출→결과 반영)를 내부에서 자동 처리하고, 완성된 텍스트 조각(AIMessageChunk)을 즉시 yield해줌." },
    { step: 8, title: "도구 자동 호출", label: "도구 자동 호출", refs: ["get_weather", "get_tourist_attractions", "get_restaurants"],
      summary: "에이전트가 필요한 도구를 스스로 골라 호출하고, 결과를 ToolMessage로 받음",
      detail: "주방장(AI)이 필요한 재료를 직접 가져오는 단계임. '날씨'를 물으면 날씨 도구만, '여행 루트'를 물으면 세 도구를 모두 호출함. ToolMessage로 결과가 오면 tool_trace에 기록해 사이드바에 보여줌." },
    { step: 9, title: "실시간 화면 렌더링", label: "실시간 화면 렌더링",
      summary: "st.write_stream()이 stream_response() 제너레이터를 받아 텍스트를 실시간으로 화면에 그림",
      detail: "글자가 생성되는 즉시 화면에 나타나는 단계임. st.write_stream()은 제너레이터(yield로 조각을 내보내는 함수)를 받아 조각이 올 때마다 화면을 업데이트하고, 전부 끝나면 누적된 전체 텍스트를 돌려줌." },
    { step: 10, title: "결과 저장·반복", label: "결과 저장·반복",
      summary: "답변을 대화 기록에 저장하고, 사용자가 새 입력을 하면 4번 단계부터 다시 진행함",
      detail: "완성된 요리를 기록해 두고 손님이 추가 주문을 기다리는 단계임. 이전 대화가 기억(messages)에 남아 있어, '거기 맛집은?' 같은 이어지는 질문도 맥락을 이해함." },
  ],
...
def get_agent():
    ...
    },
    {
      ...
      fileId: "main",
      summary: "화면을 새로 그릴 때마다 사라지지 않고 유지해야 할 데이터(대화·도구 기록 등)를 위한 빈 저장 공간을 만듦.",
...
`def initialize_session_state() -> None:
    ...
    },
    {
      ...
      name: "build_history()",
      fileId: "main",
      summary: "화면에 쌓인 대화를, AI가 알아듣는 메시지 객체 목록으로 변환함. (SystemMessage 제외)",
      how: "claude 예제의 build_messages()와 비슷하지만, 시스템 지침(SYSTEM_PROMPT)은 get_agent()에서 이미 넣었으므로 여기선 HumanMessage·AIMessage만 담음. 비용을 아끼려고 최근 10개만 포함하고, 새 입력은 stream_response()에서 따로 추가함.",
      terms: ["HumanMessage", "AIMessage", "리스트(list)", "타입 힌트"],
      lines: [
        { at: "result = []", text: "빈 목록을 만들어 두고, 아래에서 메시지 객체를 하나씩 추가함." },
...
`def build_history() -> list[Any]:
    ...
    },
    {
      ...
      fileId: "main",
      summary: "에이전트 응답을 텍스트 조각(청크)으로 실시간 전달하는 '제너레이터' 함수. 글자가 생성될 때마다 화면에 바로 표시되게 함.",
      how: "이 함수의 핵심은 yield임. 일반 함수는 return으로 결과를 한꺼번에 돌려주지만, 제너레이터는 yield로 조각을 하나씩 내보냄. agent.stream()이 ReAct 루프(도구 호출 포함)를 내부에서 자동 처리하고, 텍스트가 생성될 때마다 AIMessageChunk로 올려줌. ToolMessage가 오면 화면 출력 없이 도구 기록만 갱신함.",
      terms: ["제너레이터(Generator)", "yield", "AIMessageChunk", "ToolMessage", "stream_mode", "isinstance()", "ReAct 루프", "타입 힌트"],
      lines: [
        { at: "agent = get_agent()", text: "캐싱된(이미 만들어진) 에이전트를 가져옴." },
        { at: "tool_trace: list[dict[str, Any]] = []", text: "이번 대화에서 호출된 도구를 모아둘 빈 목록임." },
        { at: 'for chunk, metadata in agent.stream(', text: "★핵심★ agent.stream()은 ReAct 루프 전체를 처리하면서, 결과를 한 번에 주지 않고 생성되는 즉시 (청크, 메타데이터) 쌍으로 yield해줌. stream_mode=\"messages\"는 메시지 단위로 받겠다는 설정임." },
        { at: 'if isinstance(chunk, AIMessageChunk) and chunk.content:', text: "받은 청크가 AI가 생성한 텍스트 조각(AIMessageChunk)인 경우에만 yield함. ToolMessage 등 다른 종류는 건너뜀." },
        { at: 'if isinstance(chunk.content, str):', text: "텍스트가 문자열이면 바로 yield, 리스트 형태(블록 배열)이면 아래 for 블록에서 텍스트만 골라 yield함." },
        { at: 'for block in chunk.content:', text: "Claude는 content를 [{\"type\":\"text\",\"text\":\"...\"}] 형태의 블록 배열로 줄 때가 있음. type이 \"text\"인 것만 꺼내 yield함." },
        { at: 'elif isinstance(chunk, ToolMessage):', text: "도구 실행 결과(ToolMessage)가 오면 텍스트를 yield하지 않고, 도구 기록만 tool_trace에 추가함." },
        { at: "st.session_state.last_tool_trace = tool_trace", text: "스트리밍이 모두 끝난 뒤, 이번에 호출된 도구 목록을 사이드바용으로 저장함." },
      ],
      code:
    ...
    for chunk, metadata in agent.stream(
        ...
        if isinstance(chunk, AIMessageChunk) and chunk.content:
            if isinstance(chunk.content, str):
                ...
                # Claude는 content를 문자열 대신 [{"type": "text", "text": "..."}] 형태의 블록 배열로 반환함
...
`def display_chat_history() -> None:
    ...
    },
    {
      ...
      how: "화면 왼쪽의 보조 패널을 구성함. with st.sidebar 블록 안의 내용은 모두 왼쪽에 표시됨. '대화 초기화' 버튼을 누르면 저장된 대화를 비우고 st.rerun()으로 화면을 새로 그림. claude 예제와 달리 도구 인자(arguments)는 표시하지 않고 함수명과 성공/오류만 간단히 보여줌.",
      ...
      lines: [
        ...
        { at: 'st.header("직전 함수 호출")', text: "직전에 호출된 도구가 있으면 함수명과 성공/오류를 코드 블록으로 보여줌. claude 예제와 달리 인자(arguments)는 표시하지 않음." },
      ],
      code:
`def display_sidebar() -> None:
    ...
    },
    {
      ...
      name: "main()",
      fileId: "main",
      summary: "앱의 시작점. 화면을 세팅하고, 입력을 받고, 스트리밍 응답을 실시간 표시하는 전체 흐름을 지휘함.",
      how: "프로그램의 '시작 버튼'에 해당함. 페이지 설정 → 상태 초기화 → 사이드바·환영문·이전 대화 표시 → 채팅창 입력 처리 순으로 진행함. 사용자가 입력하면 st.write_stream()이 stream_response()의 청크를 실시간으로 화면에 뿌려줌. 오류가 나도 멈추지 않게 try/except로 감쌈.",
      terms: ["st.chat_input", "st.write_stream", ":= (바다코끼리)", "예외 처리(try/except)", "제너레이터(Generator)", "Streamlit"],
      lines: [
        ...
        { at: "assistant_response = st.write_stream(stream_response(user_input))", text: "★스트리밍 핵심★ st.write_stream()은 stream_response()가 yield하는 텍스트 조각을 받아 실시간으로 화면에 그려줌. 모두 끝나면 누적된 전체 텍스트를 돌려줌." },
        { at: "except Exception as exc:", text: "스트리밍 도중 오류가 나도 앱이 죽지 않도록 try/except로 감싸 오류 메시지로 대체함." },
      ],
      code:
`def main() -> None:
    """Streamlit 앱 진입점."""
...
`def _request_json(method: str, url: str, **kwargs: Any) -> dict[str, Any]:
    ...
    },
    {
      ...
      lines: [
        { at: 'display_name = place.get("displayName", {}).get(', text: ".get(\"displayName\", {})는 'displayName이 없으면 빈 딕셔너리를 쓰라'는 안전한 꺼내기임. 연달아 .get으로 더 깊은 값을 안전하게 꺼냄." },
...
def get_weather(city: str) -> dict:
    ...
    },
    {
      ...
      lines: [
        { at: "def get_tourist_attractions(city: str,", text: "@tool로 관광지 검색을 AI 도구로 등록함." },
        ...
        { at: 'return {"error": f"Google Places 관광지', text: "오류가 나면 error 항목을 담아 반환(앱이 멈추지 않음)." },
      ],
      code:
`@tool
...
`def require_api_key(env_name: str) -> str:
    ...
    // ===== common/ui_text.py (화면 문구) =====
    {
      ...
      how: "코드 곳곳에 문구를 흩어 두면 수정이 번거로움. 자주 바뀌는 안내 문구를 상수(대문자 이름)로 모아두면 한 곳만 고쳐도 전체에 반영됨. 함수는 없고 문자열 상수들만 있는 파일임. TECH_GUIDE에는 스트리밍 흐름이 설명돼 있어 claude 버전과 내용이 동일함.",
      terms: [],
      lines: [
        ...
        { at: "TECH_GUIDE =", text: "TECH_GUIDE: LangChain ReAct 스트리밍 흐름을 간단히 정리한 기술 안내." },
      ],
      code:
`APP_TITLE = "여행 플래너"
APP_ICON = "🗺️"
...
"""`,
  ...
  glossary: {
    ...
    "@st.cache_resource": "Streamlit의 캐싱 데코레이터 중 하나. 앱이 처음 뜰 때 한 번만 실행하고, 그 결과를 서버 전체에서 공유해 재사용함. AI 모델처럼 만드는 데 비용이 큰 객체에 적합함.",
    ...
    "제너레이터(Generator)": "함수 안에서 return 대신 yield로 값을 하나씩 내보내는 특별한 함수. 결과를 한꺼번에 만들지 않고 필요할 때 하나씩 생성해 주므로, 긴 텍스트를 실시간으로 표시하는 스트리밍에 적합함.",
    "yield": "제너레이터 함수에서 값을 하나 내보내는 명령. return과 달리 함수가 끝나지 않고 '일시 정지'했다가, 다음 값이 요청되면 이어서 실행됨.",
    "stream_mode": "agent.stream()에 전달하는 설정. 'messages'로 지정하면 메시지 단위(AIMessageChunk, ToolMessage 등)로 스트리밍 결과를 받음.",
    "AIMessageChunk": "스트리밍 중에 AI가 생성하는 텍스트 조각(청크). 여러 개가 순서대로 오며, 모두 이어 붙이면 완성된 AI 답변이 됨.",
