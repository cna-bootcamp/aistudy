# 여행 플래너 — LangChain + Claude (Streaming)

LangChain `create_react_agent`와 Claude `claude-sonnet-4-6`을 사용하는 스트리밍 여행 플래너 예제.  
...
- Claude는 `content`가 `list[dict]` 블록 배열로 올 수 있어 별도 처리 필요
...
| 모델별 변환 | `get_claude_tools()` 변환 함수 | 불필요 |
| 도구 루프 | `for _ in range(MAX_TOOL_ROUNDS):` 수동 루프 | `create_react_agent` 내부 자동 처리 |
| 스트리밍 | `client.messages.stream()` + `stream.text_stream` | `agent.stream(stream_mode="messages")` |
