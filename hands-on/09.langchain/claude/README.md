...
LangChain `ChatAnthropic`과 `create_react_agent`를 사용하여 여행 플래너를 구현한 예제.  
...
| 루프 처리 | `stop_reason == "tool_use"` 수동 감지 후 재호출 | `create_react_agent`가 ReAct 루프 자동 처리 |
...
| `create_react_agent` | LLM + 도구 목록을 받아 ReAct 루프를 자동 실행하는 LangGraph 그래프 컴파일러 |
