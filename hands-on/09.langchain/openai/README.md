...
LangChain `ChatOpenAI`와 `create_agent`로 구현한 여행 플래너.  
사용자가 도시명을 입력하면 날씨·관광지·맛집 도구를 자동으로 호출해 오늘의 여행 루트를 추천함.  
`create_agent`가 ReAct 루프를 자동 처리하므로 수동 tool_calls 루프 코드가 불필요함.
...
| 도구 루프 | `tool_calls` 감지 → `run_tool_call()` → 재호출 (수동 for 루프) | `create_agent`가 ReAct 루프 자동 처리 |
