...
class IndexValidator:
    ...
    def fix(self, report: ValidationReport) -> list[str]:
        ...
        - C3: 엔티티 임베딩 누락 → OpenAI로 수동 생성 (finalize_indexing)
        ...
        # 엔티티 임베딩 / community_full_content 보정은 finalize_indexing 모듈 재사용
        if "C3" in names or "W4b" in names:
            try:
                ...
                if "C3" in names and generate_missing_entity_embeddings(self.parquet_dir):
                    messages.append("C3 보정 완료: 엔티티 임베딩 생성")
                if "W4b" in names and generate_community_report_embeddings(self.parquet_dir, self.lancedb_dir):
                    messages.append("W4b 보정 완료: community_full_content 테이블 생성")
            except ImportError:
                messages.append("C3/W4b 보정 불가: finalize_indexing 모듈 없음 → python index_documents.py --force 재인덱싱 권고")
    ...
    def _check_embedding_dimension(self) -> CheckResult:
        """W6: 엔티티 임베딩 차원이 설정(1536)과 일치하는지 검증."""
        df = self._read_parquet_safe("embeddings.entity_description.parquet")
        if df is None or len(df) == 0:
...
def format_report(report: ValidationReport) -> str:
    ...
    lines = ["=" * 60, "  특허법 GraphRAG 인덱싱 검증 리포트", "=" * 60, ""]
...
def main():
    parser = argparse.ArgumentParser(description="특허법 GraphRAG 인덱싱 검증/보정 도구")
