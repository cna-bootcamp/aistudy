...
| `ENTITY_TOP_K` / `DOC_TOP_K` | `5` / `5` | 진입 엔티티 / 조문 청크 검색 수 |
| `GRAPH_EXPAND_LIMIT` | `20` | 진입 엔티티 1-hop 관계 확장 최대 건수 |
...
추론을 하게 함. 아래 두 질문은 현재 인덱싱된 특허법 KG에 **실재하는 관계 경로**를 자극함.

| 질문 | 자극하는 KG 경로 (멀티홉) |
|---|---|
| **"미성년자나 피성년후견인이 특허에 관한 절차를 밟으려면 누가 필요하고, 그 대리와 관련해 어떤 제한·자격이 적용되나요?"** | 미성년자/피성년후견인 ─[REQUIRES]→ **법정대리인** ←[APPLIES_TO]─ 행위능력 없는 자·특허취소신청 (2-hop, 다른 청크 연결) |
| **"전용실시권과 통상실시권은 특허발명에 대해 각각 어떤 관계이고, 특허발명에는 어떤 심판들이 적용되나요?"** | 전용/통상실시권 ─[APPLIES_TO]→ **특허발명** ←[APPLIES_TO]─ 권리범위확인심판·통상실시권허락심판 (허브 비교) |

> 두 질문 모두 **다리 엔티티(법정대리인·특허발명)가 질문 표면에 드러나지 않아** Vector RAG가 가장 놓치기 쉬운 유형임.

**효과 확인 방법** — 챗봇 실행 후 아래 신호를 관찰:
- `[Retrieve:GraphRAG] → N개 문서 (조문 청크 + 지식그래프)`에 **지식그래프** 문서가 포함되는지
- 답변 끝 `## 출처` → **법률** 블록에 `특허법 지식그래프(KG)` 관계 근거가 잡히는지
- `[IsSup]` 근거성 평가 reasoning이 `A ←[REL] B` 같은 그래프 관계를 인용하는지

**대조 실험**: 단일 사실 질문(예: "특허 출원 수수료는 얼마인가요?")과 위 질문을 나란히 던지면, 전자는  
지식그래프 블록이 비고 후자는 관계가 잡히는 차이로 GraphRAG 효과가 또렷이 드러남.

> **참고**: `GraphRetriever`는 1-hop 확장까지 수행하므로 다리가 1~2단계 안인 질문에서 강함.  
> 더 깊은 멀티홉은 `GRAPH_EXPAND_LIMIT`(기본 20)를 늘리거나 `_expand_graph`를 2-hop로 확장.
