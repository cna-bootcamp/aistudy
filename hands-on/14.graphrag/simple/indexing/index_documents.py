"""간단 GraphRAG 인덱싱 엔트리포인트 (특허법.pdf → Neo4j)

이중 인덱싱 파이프라인:
  [Phase 1] 특허법 조문 청크 → LLMGraphTransformer → Neo4j KG + entity_embedding 벡터 인덱스
  [Phase 2] 특허법 조문 청크 → doc_embedding 벡터 인덱스

[14.graphrag/neo4j/indexing 대비 핵심 변경 사항]
  Before: 데이터소스 = 교재(*.md)+예제코드(*.py) / 임베딩 = Ollama qwen3(4096차원)
  After : 데이터소스 = 특허법.pdf 1개 / 임베딩 = OpenAI text-embedding-3-small(1536차원)
          → 같은 조문 청크가 KG와 doc_embedding 양쪽에 동일하게 사용됨
...
# 파이썬이 모듈을 검색하는 경로 목록 맨 앞에 이 파일의 디렉터리를 추가함 (config/graph 패키지 import용)
...
# neo4j 드라이버의 스키마 알림(예: 속성 미존재)은 의도된 상황이라 ERROR 이상만 표시
logging.getLogger("neo4j").setLevel(logging.ERROR)
logging.getLogger("neo4j.notifications").setLevel(logging.ERROR)
logger = logging.getLogger(__name__)

# --mode test에서 인덱싱할 청크 수 (소량 검증용)
_TEST_CHUNK_LIMIT = 20


def parse_args() -> argparse.Namespace:
    """명령줄 인자 파싱 (--force / --mode)"""
    parser = argparse.ArgumentParser(description="간단 GraphRAG 인덱싱 (특허법.pdf → Neo4j)")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="full=전체 인덱싱, test=소량 테스트(앞 20개 청크)",
    )
    return parser.parse_args()


def log_resolved_paths(settings: Settings) -> None:
    """LLM·Neo4j를 띄우기 전에 도출된 경로를 먼저 출력해 경로 오류를 조기 발견함"""
    logger.info("-" * 60)
    logger.info("[경로 확인] 워크스페이스 루트 : %s", settings.workspace_root)
    logger.info("[경로 확인] 특허법 PDF       : %s (존재=%s)", settings.pdf_path, settings.pdf_path.exists())
    ...
    if not settings.pdf_path.exists():
        logger.warning("특허법 PDF가 없습니다 — 경로를 먼저 확인하세요.")
...
def main() -> None:
    ...
    args = parse_args()
    logger.info("=" * 60)
    logger.info("간단 GraphRAG 인덱싱 시작 (mode=%s, force=%s)", args.mode, args.force)
    logger.info("=" * 60)
    ...
    logger.info("인덱싱 대상 청크: %d개", len(documents))
    ...
    if documents and kg_result["extracted_nodes"] == 0:
        # 대상 문서가 있었는데 엔티티가 0개면 추출 경로 실패 — 명확히 경고
