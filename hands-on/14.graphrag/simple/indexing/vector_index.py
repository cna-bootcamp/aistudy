"""Neo4j 벡터 인덱스 관리 모듈 (OpenAI 임베딩)

Neo4j는 KG와 벡터 인덱스를 하나의 DB에 통합 저장함 (별도 벡터 저장소 불필요).
이 모듈이 만드는 벡터 인덱스 2개:
- entity_embedding: 특허법 엔티티의 id+description 임베딩 (KG 노드 대상, from_existing_graph)
- doc_embedding   : 특허법 조문 청크 텍스트 임베딩 (신규 Chunk 노드, from_documents)

[14.graphrag/neo4j/indexing 대비 변경] 임베딩을 Ollama qwen3-embedding(4096차원)에서
OpenAI text-embedding-3-small(1536차원)으로 교체해 agentic-rag 챗봇과 임베딩을 통일함.
"""
import logging
...
# OpenAIEmbeddings: OpenAI 임베딩 API 래퍼 (OPENAI_API_KEY 환경변수를 자동 참조)
...
class VectorIndexManager:
    ...
    def __init__(self, settings: Settings):
        ...
        # text-embedding-3-small(1536차원) — 인덱스 차원과 질의 임베딩 차원이 반드시 일치해야 검색이 동작함
    ...
    def create_doc_vector_index(self, documents: list[Document]) -> Neo4jVector:
        ...
        → "특허 요건은?" 같은 질문을 조문 원문 단위 벡터 유사도로 직접 매칭함
        """
