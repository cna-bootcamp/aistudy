...
특허법 청크에서 엔티티/관계를 추출해 Neo4j Knowledge Graph를 구축함.

핵심 설정:
- strict_mode=False: allowed_* 목록을 힌트로만 사용해 그 외 엔티티/관계도 허용 (재현율 우선)
- ignore_tool_usage=False: gpt-oss-120b는 네이티브 함수호출이 동작하므로 node_properties(description) 추출 활성
...
class KGBuilder:
    """LLMGraphTransformer로 특허법에서 KG를 구축하는 빌더 (비동기 배치)

    LLMGraphTransformer가 만드는 노드 구조:
    - 라벨: __Entity__ 공통 라벨 + 엔티티 타입 라벨(Requirement, Procedure 등)
    ...
    def __init__(self, settings: Settings, graph: Neo4jGraph):
        ...
        # - 프롬프트 추출 모드: node_properties 생략 (gpt-oss-20b 등 폴백 경로)
    ...
    def build_from_documents(self, documents: list[Document]) -> dict:
        """특허법 청크에서 KG를 구축함 (동기 인터페이스로 비동기 처리를 감쌈)
    ...
    async def _build_async(self, documents: list[Document]) -> dict:
        ...
        for start in batch_starts:
            ...
            try:
                ...
                if not extracted_nodes:
                    logger.warning("청크 %d-%d: 엔티티 0개 추출 (LLM 추출 실패 또는 빈 청크)",
                                   start + 1, batch_end)
        ...
        logger.info("KG 구축 완료: 성공 %d, 실패 %d (총 %d), 추출 엔티티 %d개",
