"""LangChain + Neo4j GraphRAG 검색 엔진.

`entity_embedding`·`doc_embedding` 벡터 검색, `GraphCypherQAChain` 그래프 질의,
벡터 시드 기반 1-hop 그래프 확장, 사용자 Cypher 직접 실행을 제공함.
"""
import logging
import re
...
class QueryEngine:
    ...
    def search(self, question: str, mode: str, history: list[dict[str, str]] | None = None) -> dict[str, Any]:
        """선택된 모드로 검색 실행."""
        if mode == "vector":
            return self.vector_search(question, history)
        if mode == "graph_qa":
            return self.graph_qa(question)
        if mode == "hybrid":
            return self.hybrid_search(question, history)
        if mode == "cypher":
            return self.cypher_direct(question)
        return self.vector_search(question, history)

    def vector_search(self, question: str, history: list[dict[str, str]] | None = None) -> dict[str, Any]:
    ...
    def graph_qa(self, question: str) -> dict[str, Any]:
        ...
        gpt-oss-20b는 간헐적으로 Cypher 대신 안내 문장("Please provide the question ...")을 생성해
        Neo4j SyntaxError를 유발하는 비결정성이 있음. LLM API 재시도(max_retries)는 이런 다운스트림
        Cypher 실행 오류를 잡지 못하므로, 여기서 체인을 한 번 더 재시도하고(전이적 실패 보정),
        그래도 실패하면 집계(count) 질문은 정답이 결정적이라 Cypher 집계 폴백으로 복구함.
        """
    ...
    def hybrid_search(self, question: str, history: list[dict[str, str]] | None = None) -> dict[str, Any]:
...
Answer in Korean using only the query result. If the result is empty, say that no matching graph data was found."""
    ...
    def _try_graph_aggregate_fallback(
        ...
        """LLM Cypher가 집계 질문을 놓친 경우 Cypher 집계로 보정."""
    ...
    def _build_vector_context(self, entity_hits: list[dict[str, Any]], doc_hits: list[dict[str, Any]]) -> str:
        """엔티티 결과와 문서 청크 결과를 LLM 컨텍스트로 변환."""
        parts: list[str] = []
        if entity_hits:
    ...
    def _build_hybrid_context(self, seed_entities: list[dict[str, Any]], graph_rows: list[dict[str, Any]]) -> str:
        """벡터 시드 엔티티와 1-hop 관계를 통합 컨텍스트로 변환."""
    ...
    def _generate_answer(self, question: str, context: str, history: list[dict[str, str]] | None = None) -> str:
        """Groq LPU LLM으로 컨텍스트 기반 답변 생성."""
        ...
        try:
            ...
            if history:
                for msg in history[-4:]:
                    if msg["role"] == "user":
                        history_msgs.append(HumanMessage(content=msg["content"][:500]))
                    elif msg["role"] == "assistant":
                        history_msgs.append(AIMessage(content=msg["content"][:500]))
            response = chain.invoke({"question": question, "context": context, "history": history_msgs})
    ...
    def _extract_intermediate_value(steps: list[dict[str, Any]], key: str) -> Any:
        """GraphCypherQAChain 중간 단계에서 특정 키 값 추출."""
        for step in steps:
            if key in step:
                return step[key]
        return None
