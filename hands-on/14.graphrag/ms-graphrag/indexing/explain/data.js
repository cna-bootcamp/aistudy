window.EXPLAIN_DATA = {
  ...
  flow: [
    { step: 1, title: "설정 로드",        label: "설정 로드",        refs: ["ms_indexing_settings_settings"], summary: "settings.py가 경로와 모델명을 확정함",               detail: "작업 폴더, 교재 위치, GraphRAG Parquet 저장소, LanceDB 저장소, Groq/Ollama 모델명을 한 객체에 모읍니다. 모든 경로는 __file__ 위치에서 자동 계산되므로 OS나 Clone 위치가 달라도 수정 없이 동작합니다." },
    { step: 2, title: "문서 수집",        label: "문서 수집",        refs: ["ms_indexing_loader_graphragdocumentloader_load_textbook", "ms_indexing_loader_graphragdocumentloader_load_code", "ms_indexing_loader_graphragdocumentloader_split_text_by_sections", "ms_indexing_loader_graphragdocumentloader_split_python_code"], summary: "GraphRAGDocumentLoader가 교재와 예제코드를 청킹함",  detail: "교재는 마크다운 헤더(#~####) 기준으로 섹션을 나누고 500자 미만 소섹션은 합쳐 적정 크기 청크를 만듭니다. 예제코드는 ast.parse()로 문법 구조를 읽어 함수·클래스 단위로 자릅니다. 글자 수로만 자르면 함수 중간이 잘리므로 AST를 씁니다." },
    { step: 3, title: "교재 입력 준비",   label: "교재 입력 준비",   refs: ["ms_indexing_main_prepare_input_documents", "ms_indexing_loader_graphragdocument_to_text"], summary: "교재 청크를 data/input/*.txt로 내보냄",              detail: "Microsoft GraphRAG CLI는 data/input 디렉터리의 .txt 파일만 입력으로 읽습니다. 기존 파일을 모두 지운 뒤 메타데이터 헤더(Source/File/Section)와 본문을 합쳐 한 청크 = 한 파일로 저장합니다." },
    { step: 4, title: "GraphRAG CLI 실행", label: "GraphRAG CLI 실행", refs: ["ms_indexing_main_run_graphrag_index", "ms_indexing_main_progresstracker_process_line"], summary: "python -m graphrag index --root indexing 실행",       detail: "현재 가상환경의 graphrag를 python -m graphrag로 실행합니다. PATH의 graphrag.exe가 다른 인터프리터를 가리켜도 영향을 받지 않습니다. CLI가 엔티티·관계·커뮤니티를 추출해 Parquet 파일과 LanceDB 벡터 인덱스를 생성합니다." },
    { step: 5, title: "후처리",           label: "임베딩 후처리",    refs: ["ms_indexing_finalize_finalize_indexing", "ms_indexing_finalize_generate_missing_entity_embeddings", "ms_indexing_finalize_generate_community_report_embeddings"], summary: "엔티티와 community_full_content 임베딩 누락을 보완함", detail: "GraphRAG CLI 중 Ollama OOM이나 타임아웃으로 일부 임베딩이 빠질 수 있습니다. finalize_indexing이 누락분을 Ollama로 다시 생성하고, DRIFT Search 진입점인 community_full_content LanceDB 테이블을 만듭니다." },
    { step: 6, title: "코드 인덱싱",      label: "코드 인덱싱",      refs: ["ms_indexing_code_index_code", "ms_indexing_code_build_code_vector_index", "ms_indexing_code_get_ollama_embedding"], summary: "예제코드는 KG 없이 코드 전용 벡터 인덱스에 저장함",  detail: "절차적인 코드에는 개념 관계 그래프보다 유사도 검색이 적합합니다. AST 청킹된 코드 청크를 Ollama qwen3-embedding으로 벡터화해 LanceDB code_chunks 테이블에 저장합니다." }
  ],
  functions: [
    {
