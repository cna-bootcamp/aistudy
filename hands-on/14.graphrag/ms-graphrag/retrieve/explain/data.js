window.EXPLAIN_DATA = {
  ...
  flow: [
    { step: 1, title: "앱 시작·설정 로드",   summary: "사이드바에서 검색 모드, LLM 모델, 세부 옵션 선택",          detail: "render_sidebar()가 검색 모드(Auto/Basic/Local/Global/DRIFT/Code), LLM 모델명, community level, DRIFT 재시도 수, Code Top-K 같은 옵션을 사이드바에 표시합니다. 사용자가 바꾸지 않으면 .env와 settings 기본값이 사용됩니다." },
    { step: 2, title: "산출물 검증",          summary: "Parquet·LanceDB·.env 파일 존재 여부 사전 확인",            detail: "validate_paths()가 엔티티·커뮤니티·텍스트 유닛 등 Parquet 5개, GraphRAG LanceDB 벡터 디렉터리, 코드 LanceDB 디렉터리, .env 파일이 모두 있는지 확인합니다. 하나라도 없으면 st.stop()으로 앱을 중단하고 빠진 경로를 화면에 표시합니다." },
    { step: 3, title: "질문 재작성",          summary: "대화 이력이 있으면 후속 질문을 독립 질문으로 바꿈",          detail: "condense_question()이 '그건 왜 그래?' 같은 타원형 후속 질문을 '로컬 서치의 entity 탐색 방식은 왜 그래?' 처럼 맥락 없이도 의미가 완성된 독립 질문으로 재작성합니다. 임베딩·검색 정확도를 높이는 핵심 전처리입니다." },
    { step: 4, title: "라우팅",               summary: "Auto이면 키워드 규칙 → LLM fallback으로 검색 모드 결정",   detail: "QueryRouter.route()가 수동 선택이면 그대로 쓰고, Auto이면 먼저 키워드 규칙으로 5가지 모드를 점수화합니다. 확신도가 min_confidence(기본 0.65) 미만이면 LLM few-shot으로 한 번 더 판단을 받습니다." },
    { step: 5, title: "검색 실행",            summary: "모드에 따라 GraphRAG API 또는 코드 LanceDB 검색 수행",      detail: "code 모드면 CodeVectorRetriever가 Ollama 임베딩 → LanceDB 벡터 검색 → LLM 답변 합성 순서로 처리합니다. 나머지 모드(basic/local/global/drift)는 GraphRAGRetriever가 해당 GraphRAG API를 비동기로 호출합니다. DRIFT 실패 시에는 Local Search로 자동 폴백합니다." },
    { step: 6, title: "결과 표시",            summary: "답변·사용 모드·라우팅 이유·출처를 화면에 표시",             detail: "render_result()가 LLM 답변과 함께 실제 사용된 모드, 라우팅 확신도, fallback 여부를 캡션으로 보여줍니다. '출처 및 라우팅 근거' 확장 패널에서 엔티티·커뮤니티 리포트·텍스트 유닛 등 GraphRAG가 실제로 참조한 원문을 확인할 수 있어 교육 용도로 유용합니다." }
  ],
  functions: [
    {
