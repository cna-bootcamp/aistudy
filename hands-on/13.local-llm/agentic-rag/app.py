#!/usr/bin/env python3
"""특허/지식재산권 Agentic RAG 챗봇 (LangGraph StateGraph + 로컬 LLM gemma3:12b)
...
[12.web-youtube-search/agentic-rag 대비 핵심 변경 사항]
  Before: 클라우드 LLM ChatGroq(openai/gpt-oss-120b) / YouTube Data API v3(API 키 필요)
  After : 로컬 LLM ChatOllama(gemma3:12b, Ollama 런타임)로 추론 → API 키·비용 없이 오프라인 추론
          YouTube 검색을 scrapetube 스크래핑으로 대체(API 키 불필요) + 유효하지 않은 영상 주소 검증
          질의어를 소스별(vectordb·web·youtube)로 각각 최적화해 검색 정확도를 높임
  유지   : 질의 임베딩은 기존 공용 벡터DB(1536차원)와의 호환을 위해 OpenAI text-embedding-3-small 사용
          (생성 LLM만 로컬로 전환함. 임베딩까지 로컬화하려면 별도 로컬 벡터DB 재구축이 필요함)

[Reflection / 라우팅 신호]
  Route   : 특허/지식재산권 질문인지(검색 필요) + 어떤 소스로 검색할지 + 소스별 최적 쿼리 판단
...
사전 요구사항:
  1) Ollama 서버 실행 + 모델 다운로드: `ollama serve` 후 `ollama pull gemma3:12b`
  2) 특허법 공용 벡터 DB(hands-on/10.rag/vectordb, 컬렉션 patent_law)가 존재해야 함
...
# ChatOllama: 로컬 Ollama 런타임의 채팅 모델 래퍼 (llm.invoke()로 대화 요청 전송, API 키 불필요)
from langchain_ollama import ChatOllama
from langchain_chroma import Chroma                       # ChromaDB 벡터 스토어 래퍼
from langchain_openai import OpenAIEmbeddings             # OpenAI 임베딩 모델 (질의 벡터화 전용)
...
# .env에서 OPENAI_API_KEY(질의 임베딩)를 로드함. LLM은 로컬 Ollama라 별도 키가 필요 없음
...
LLM_MODEL = "gemma3:12b"                      # Ollama 런타임에서 서빙하는 로컬 LLM (구글 Gemma 3 12B)
OLLAMA_BASE_URL = os.environ.get("OLLAMA_BASE_URL", "http://localhost:11434")  # Ollama 서버 주소
TOP_K = 5                                    # 벡터DB 유사도 검색 시 가져올 문서 수
WEB_MAX_RESULTS = 5                          # 웹(DuckDuckGo) 검색 결과 수 (MUST: 최신 5개)
YOUTUBE_MAX_RESULTS = 5                      # 최종 유지할 유효한 YouTube 영상 수
YOUTUBE_SCRAPE_LIMIT = 12                    # scrapetube에서 가져올 후보 수 (유효성 검사 탈락분을 감안해 여유 확보)
...
class RouteDecision(BaseModel):
    """라우터 결과: 검색 필요 여부 + 선택 소스 + 소스별 최적 쿼리 (검색 방식별 질의어 최적화)."""
...
def search_web(query: str) -> list[dict]:
    ...
    for item in raw_results:
        link = item.get("link", "")
        # 웹 결과에 섞여 들어온 YouTube 페이지는 제외함. 영상은 scrapetube 검색이 담당하므로,
...
def _extract_runs_text(node: Optional[dict]) -> str:
    ...
    YouTube 내부 JSON은 텍스트를 단순 문자열이 아니라 runs 배열로 쪼개 담는 경우가 많아,
    각 run의 text를 이어 붙여 사람이 읽는 한 문장으로 복원함.
    """
    if not node:
        return ""
...
def is_valid_video(url: str) -> bool:
    """YouTube oembed 엔드포인트로 영상 주소가 유효(공개·재생 가능)한지 확인함 (유효하지 않은 영상 주소 체크).

    oembed는 공개 영상이면 200(JSON 메타데이터)을, 비공개·삭제·존재하지 않는 영상이면 401/404를 반환함.
    API 키 없이 동작하며, 스크래핑 결과에 섞일 수 있는 유효하지 않은 주소를 출처 표기 전에 걸러냄 (MUST).
    """
...
def search_youtube(query: str) -> list[dict]:
    """scrapetube로 YouTube 영상을 검색하고 유효하지 않은 주소를 걸러 제목·채널·URL을 반환함.

    scrapetube.get_search(query, limit, sort_by): YouTube 검색 결과 페이지를 스크래핑해 영상
    메타데이터 제너레이터를 반환함(API 키 불필요). Data API v3와 달리 publishedAfter 같은 날짜
    필터를 지원하지 않으므로, sort_by="relevance"로 관련도 순 결과를 모으고 게시 시점은 상대 표기
    (publishedTimeText, 예: '3 weeks ago')만 보존함. 각 영상은 is_valid_video()로 유효성을 확인해
    유효한 영상만 최대 YOUTUBE_MAX_RESULTS개 반환함.
    """
    results: list[dict] = []
    # get_search는 제너레이터를 반환하므로, 유효성 검사 탈락을 감안해 limit을 여유 있게 잡고 순회함
    ...
    for video in videos:
        ...
        # 유효하지 않은(비공개·삭제·존재하지 않는) 영상 주소를 oembed로 걸러냄 (MUST)
        ...
        # 유효한 영상이 목표 개수에 도달하면 즉시 중단해 불필요한 oembed 호출을 줄임
...
def build_context(state: AgentState) -> str:
    ...
    if state["youtube_results"]:
        ...
        for i, item in enumerate(state["youtube_results"], 1):
            ...
            yt_parts.append(f"[영상 {i}] {item['title']} ({item['channel']}{published})\nURL: {item['url']}")
        sections.append("=== YouTube 검색 결과 (scrapetube) ===\n" + "\n\n".join(yt_parts))
...
def build_sources_section(state: AgentState) -> str:
    ...
    # YouTube 출처: 제목 + URL 링크 (반드시 포함, 유효성 검증을 통과한 주소만 표기됨)
    if state["youtube_results"]:
...
class AgenticRAG:
    ...
    def __init__(self, llm: ChatOllama, vectorstore: Chroma):
        self.llm = llm
...
[검색 방식별 질의어 최적화 규칙] (반드시 수행 — 소스마다 검색 엔진 특성이 달라 쿼리를 각각 따로 최적화)
  - vectordb_query : 특허법 '조문' 검색용. 구어체를 정확한 법률 용어로 바꿔 작성 (예: '특허 등록 요건 신규성 진보성').
  - web_query      : 웹 검색용 핵심 키워드. 비용·통계·실무 등 최신 정보에 맞춤. 연도·시간 표현(2024 등)은 절대 금지.
  - youtube_query  : 영상 검색용. 쉼표 없이 짧고 구체적인 키워드로 작성 (예: '특허 출원 방법').
  - 세 쿼리는 같은 질문이라도 소스 특성에 맞게 서로 다르게 다듬는 것이 원칙임.
    예) 질문 "내가 만든 앱 아이디어 특허로 보호받는 방법이랑 비용, 관련 영상도 보여줘"
      → vectordb_query: '특허 등록 요건 및 출원 절차'
      → web_query: '앱 아이디어 특허 출원 비용'
      → youtube_query: '앱 특허 출원 방법'
    (단, '특허 요건'처럼 이미 짧은 법률 키워드면 세 쿼리가 비슷해질 수 있음 — 억지로 다르게 만들지 말 것)
  - 검색 불필요(False)면 세 쿼리 모두 빈 문자열로 두세요.
...
판단 근거(reasoning)는 한국어로 작성하세요."""),
        ...
        if sources:
            print(f"  → vectordb_query: '{decision.vectordb_query}'")
...
def run_demo(agent: AgenticRAG) -> None:
    ...
    로컬 LLM은 질의당 여러 번 호출(라우팅·평가·생성)되어 클라우드보다 느리므로,
    데모 질의는 멀티소스 검색 1건 + 비특허 직접 답변 1건으로 간결히 구성함.
    """
    demo_questions = [
        "특허 요건에 대해 법률과 영상을 검색해서 알려줘",   # 벡터DB + YouTube(scrapetube) 통합
...
def chat(agent: AgenticRAG) -> None:
    ...
    print("\n" + "=" * 60)
    print("특허/지식재산권 Agentic RAG 챗봇 (로컬 LLM gemma3:12b)")
    print("=" * 60)
