#!/usr/bin/env python3
"""특허법 PDF 인덱싱 파이프라인 (KaLM-Embedding 4-bit, MRL 1024차원 공용 벡터 DB 구축)

PDF 로드 → 전처리 → 청킹 → 임베딩 → ChromaDB 저장의 인덱싱 단계를 수행함.
문서검색(retrieve) 프로그램이 공유하는 벡터 DB(컬렉션 patent_law_kalm1024)를 생성함.

[10.rag/indexing.py 대비 변경점]
  Before: OpenAI text-embedding-3-small (1536차원, 클라우드 API)
  After : 로컬 KaLM-Embedding-Gemma3-12B-2511 (4-bit, MRL 1024차원). 임베딩·차원·지시문은 common.py로 공유
  전처리·청킹 로직(clean_text, LAW_SEPARATORS 등)은 10.rag/indexing.py에서 재사용함

사용법:
    python indexing.py            # ../data 대신 10.rag/data의 특허법 PDF를 인덱싱하여 공용 벡터 DB 생성
"""
...
# 공유 계약(모델·차원·지시문·컬렉션명·경로)과 KaLM 임베딩 래퍼를 단일 출처에서 가져옴
from common import COLLECTION_NAME, EMBED_DIM, VECTORDB_DIR, KaLMEmbeddings

# ---------------------------------------------------------------------------
# 경로 설정 (이식성을 위해 모든 경로를 __file__ 기준으로 도출)
# ---------------------------------------------------------------------------
SCRIPT_DIR = Path(__file__).resolve().parent    # 이 파일이 위치한 디렉터리(indexing/)를 절대경로로 구함
# indexing/ → agentic-rag-chat/ → 13.local-llm/ → hands-on/ 순으로 상위 3단계가 hands-on 루트임
HANDS_ON_DIR = SCRIPT_DIR.parent.parent.parent
DATA_DIR = HANDS_ON_DIR / "10.rag" / "data"      # 인덱싱 대상 PDF 디렉터리 (특허법.pdf 재사용)

# ---------------------------------------------------------------------------
# 상수 정의 (10.rag/indexing.py와 동일한 전처리·청킹 파라미터 재사용)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 1. 전처리 (10.rag/indexing.py clean_text 재사용)
# ---------------------------------------------------------------------------
...
def filter_chunks(chunks: list) -> list:
    ...
    "[전문개정 ...]" 같은 개정 태그만 담긴 청크는 질의와 무관하게 상위에 노출되어 검색 품질을
    떨어뜨리므로 임베딩 대상에서 제외함. (clean_text가 제거하지 못한 잔여 노이즈 정리)
    """
    kept = []
    dropped = []
    for chunk in chunks:
    ...
    if dropped:
        print(f"  - 노이즈 청크 제거: {len(dropped)}개")
    return kept
...
def build_vectordb(chunks: list):
    ...
    Chroma.from_documents: 문서 리스트를 embedding(여기선 KaLMEmbeddings)으로 벡터화해 컬렉션에 저장하는 헬퍼.
    """
...
def verify_vectordb(vectorstore) -> None:
    """저장된 벡터 수·임베딩 차원을 출력하고 테스트 쿼리로 on-topic 검색 동작을 확인함."""
    # ._collection.count(): 컬렉션에 실제 저장된 벡터 개수를 반환함
    count = vectorstore._collection.count()
    print(f"  - 저장된 벡터 수: {count}")

    # 임베딩 함수로 쿼리 한 건을 벡터화하여 차원이 1024(MRL 절단값)인지 확인함
    test_query = "특허를 받을 수 있는 요건은?"
    query_vector = vectorstore._embedding_function.embed_query(test_query)
    print(f"  - 임베딩 차원: {len(query_vector)} (기대 {EMBED_DIM})")

    # 영속 DB에 대해 유사도 검색이 동작하고 결과가 질의와 관련 있는지(on-topic) 상위 5건으로 확인함
...
def main() -> None:
    ...
    print("[4/5] 임베딩 + ChromaDB 저장 (KaLM 4-bit 모델 적재 ~7GB VRAM, 최초 1회)")
